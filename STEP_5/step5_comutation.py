...
os.makedirs("comutation_plots", exist_ok=True)
comut_records = []
...
for geneA, geneB in comut_pairs:
    ...
    plt.tight_layout()
    plt.savefig(f"comutation_plots/{geneA}_{geneB}_survival_curve.png", dpi=300)
    plt.close()
...
if comut_records:
    ...
    comut_df.to_csv("step5_comutation_hazard_ratios.csv", index=False)
